...
if BUILD_WITH_SYSTEM_OPENSSL:
    ...
    SSL_INCLUDE = (
        os.path.join("/data/data/com.termux/files/usr", "include", "openssl"),
    )
...
if BUILD_WITH_SYSTEM_CARES:
    CORE_C_FILES = filter(lambda x: "third_party/cares" not in x, CORE_C_FILES)
    CARES_INCLUDE = (
        os.path.join("/data/data/com.termux/files/usr", "include"),
    )

if BUILD_WITH_SYSTEM_RE2:
    CORE_C_FILES = filter(lambda x: "third_party/re2" not in x, CORE_C_FILES)
    RE2_INCLUDE = (
        os.path.join("/data/data/com.termux/files/usr", "include", "re2"),
    )
...
if BUILD_WITH_SYSTEM_ABSL:
    EXTENSION_LIBRARIES += tuple(
        lib.stem[3:]
        for lib in sorted(
            pathlib.Path("/data/data/com.termux/files/usr").glob(
                "lib*/libabsl_*.so"
            )
        )
    )
